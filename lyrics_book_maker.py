...
def parse_japanese_lrc(file_path):
    ...
    try:
        ...
        for line in lines:
            ...
            # 跳过元数据行
            if not line or (
                    line.startswith('[') and ('作词' in line or '作曲' in line or '编曲' in line or 'by:' in line)):
                continue
    ...
    try:
        ...
        for line in lines:
            ...
            if not line or (line.startswith('[') and (
                    '作词' in line or '作曲' in line or '编曲' in line or '制作人' in line or 'by:' in line)):
                continue
...
def create_lyrics_book():
    ...
    try:
        ...
        for idx, file_path in enumerate(lrc_files):
            ...
            if lrc_type == "no":
                print(f"警告: '{song_title}' 的歌词是纯音乐，跳过处理")
                continue
            if not lrc_type:
        ...
        print(
            f"包含歌曲: 日文 {lang_count['japanese']} 首, 中文 {lang_count['chinese']} 首, 英文 {lang_count['english']} 首")
        print("=" * 50)
        print("\n打印提示：")
        print("1. 纸张大小: A5")
